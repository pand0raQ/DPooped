...
class SyncService {
    ...
    func syncWalk(_ walk: Walk) {
            cloudKitService.saveWalk(walk) { result in
                switch result {
                case .success(let recordID):
    ...
    private func updateLocalWalks(with cloudWalks: [Walk], for dog: Dog) {
        ...
        do {
            let localWalks = try context.fetch(FetchDescriptor<Walk>())
            let dogWalks = localWalks.filter { $0.dog?.id == dog.id }
            
            for cloudWalk in cloudWalks {
                if let localWalk = dogWalks.first(where: { $0.cloudKitRecordID == cloudWalk.cloudKitRecordID }) {
